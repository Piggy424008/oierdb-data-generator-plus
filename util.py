...
award_levels = ['金牌', '银牌', '铜牌', '一等奖', '二等奖', '三等奖',
...
def __main__():
    ...
    with open('static/contests.json', encoding="utf8") as f:
        for contest in json.load(f):
            Contest.create(contest)

    with open('static/grades.json', encoding="utf8") as f:
        g_ = json.load(f)
    g_initial = g_['initial']
    g_element = g_['element']
    g_special = g_['special']

    with open('static/surnames.json', encoding="utf8") as f:
        surnames = json.load(f)

    with open('static/scoring.json', encoding="utf8") as f:
        scoring = json.load(f)
    ...
    def enrollment_middle(contest, grades):
        ...
        while mask:
            ...
            ems[year - grade +
                1] = (1 if is_primary_or_none and grade > 5 else 2)
            mask &= mask - 1
        return ems
    ...
    def rank_coefficient(rank, total, name=None):
        ...
        if not (1 <= rank <= total):
            print(
                f'\x1b[01;33mwarning: \x1b[0m诡异的排名：\x1b[32m{rank}\x1b[0m / \x1b[32m{total}\x1b[0m (from \x1b[32m{name}\x1b[0m)，已自动 clamped', file=stderr)
    ...
    def contest_type_coefficient(type, name=None):
        ...
        if type not in scoring:
            print(
                '\x1b[01;33mwarning: \x1b[0m未知的比赛类型：\x1b[32m\'{type}\'\x1b[0m (from \x1b[32m{name}\x1b[0m)，不计算贡献', file=stderr)
