...
def __main__():
    ...
    def parse_school():
        ...
        with open('data/school.txt', encoding="utf8") as f:
        ...
        for idx, line in tqdm(enumerate(raw_data), total=len(raw_data)):
            try:
                ...
                print(
                    f'\x1b[01mschool.txt:{idx + 1}: \x1b[031merror: \x1b[0;37m\'{line.strip()}\'\x1b[0m，{e}', file=stderr)
    ...
    def parse_raw():
        ...
        for idx, line in tqdm(enumerate(raw_data), total=len(raw_data)):
            try:
                ...
                print(
                    f'\x1b[01mraw.txt:{idx + 1}: \x1b[31merror: \x1b[0;37m\'{line.strip()}\'\x1b[0m，{e}', file=stderr)
    ...
    def attempt_merge(threshold=240):
        ...
        for idx, oier in tqdm(enumerate(OIer.get_all()), total=OIer.count_all()):
            ...
            if '--show-incomplete-merge' in argv and len(a) != 1:
                print(
                    f'\x1b[01;33mwarning: \x1b[0;32m\'{oier.name}\'\x1b[0m 未完全合并，合并进度为 \x1b[32m{original_length}\x1b[0m → \x1b[32m{len(a)}\x1b[0m', file=stderr)
            recordseqs.extend(a)
        OIer.clear()
        for recordseq in tqdm(recordseqs):
            ...
            # 入学年份取众数，相同的话取最早的
            em = util.get_weighted_mode(
                [record.ems for record in recordseq if not record.is_keep_grade()])[0]
    ...
    def merge_schools():
        ...
        with open('dist/merge_preview.txt', 'w', encoding="utf8") as f:
            print('''# 用 '#' 号表示注释。
        ...
        #   c,<province>,<city>,<name>，表示插入学校 <province>,<city>,<name>。''', file=f)
            for province, school_name in tqdm(new_schools):
                ...
                if method == 'b':
                    school = res[1]
                    print(
                        f'\x1b[32m[direct redirect]\x1b[0m: \x1b[35m\'{school_name}\'\x1b[0m → \x1b[37m\'{school.name}\'\x1b[0m', file=stderr)
                    print(f'b {school_name} {school.name}', file=f)
                elif method == 'f':
                    school = res[1]
                    print(
                        f'\x1b[32m[name changed]\x1b[0m: \x1b[35m\'{school_name}\'\x1b[0m ← \x1b[37m\'{school.name}\'\x1b[0m', file=stderr)
                    print(f'f {school_name} {school.name}', file=f)
                elif method == 'fs':
                    school = res[1]
                    standard = res[2]
                    print(
                        f'\x1b[32m[towards standard name]\x1b[0m: (\x1b[35m\'{school_name}\'\x1b[0m, \x1b[37m\'{school.name}\'\x1b[0m) → \x1b[33m\'{standard}\'\x1b[0m', file=stderr)
                    print(f'f {standard} {school.name}', file=f)
                    print(f'b {school_name} {school.name}', file=f)
                elif method == 'c':
                    city = res[1]
                    print(
                        f'\x1b[32m[create]\x1b[0m: (\x1b[35m\'{province}\'\x1b[0m, \x1b[35m\'{city}\'\x1b[0m, \x1b[35m\'{school_name}\'\x1b[0m)', file=stderr)
    ...
    def output_schools():
        ...
        with open('dist/school.json', 'w', encoding="utf8") as f:
    ...
    def output_compressed():
        ...
        OIer.sort_by_score()
        with open('dist/result.txt', 'w', encoding="utf8") as f:
            for oier in tqdm(OIer.get_all()):
    ...
    def compute_sha512():
        ...
        with open('dist/result.info.json', 'w', encoding="utf8") as f:
            print('{"sha512":"' + sha512 + '", "size":' +
                  str(file_size) + '}', file=f)

    def update_static():
        '调用 update_static.js 以产生静态 JSON 信息。'
        command = 'update_static.js' if sys.platform == 'win32' or sys.platform == 'win64' else './update_static.js'
        os.system(command)
    ...
    def compute_contest():
        for oier in tqdm(OIer.get_all()):
            for i in oier.records:
                rname = i.to_highly_compressed_format(oier.enroll_middle)
                with open(f"./dist/contest/{rname}.jsonl", "a") as f:
                    f.write(f"{oier.uid}\n")
