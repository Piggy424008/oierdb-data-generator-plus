        ...
        if result := re.match(__re_score_with_rank__, score):
            ...
            if len(self.contestants) == 0:
                if not (score is None) and score > self.full_score:
                    print(
                        f'\x1b[01;33mwarning: \x1b[0m超过满分的分数：\x1b[32m{score}\x1b[0m > \x1b[32m{self.full_score}\x1b[0m，于比赛 \x1b[32m\'{self.name}\'\x1b[0m', file=stderr)
                rank = 1
                ...
                if (score is None) or (self.contestants[-1].score is None):
                    print(
                        f'\x1b[01;33mwarning: \x1b[0m不兼容的分数：\x1b[32m{score}\x1b[0m > \x1b[32m{self.contestants[-1].score}\x1b[0m，于比赛 \x1b[32m\'{self.name}\'\x1b[0m', file=stderr)
                elif score > self.contestants[-1].score:
                    print(
                        f'\x1b[01;33mwarning: \x1b[0m不单调的分数：\x1b[32m{score}\x1b[0m > \x1b[32m{self.contestants[-1].score}\x1b[0m，于比赛 \x1b[32m\'{self.name}\'\x1b[0m', file=stderr)
                rank = len(self.contestants) + 1
