...
class Record:
    ...
    def __province_format__(province):
        try:
            ...
            print(
                f'\x1b[01;33mwarning: \x1b[0m未知的省级行政区：\x1b[0;32m\'{province}\'\x1b[0m', file=stderr)
            return province
    ...
    def __award_level_format__(level):
        try:
            ...
            print(
                f'\x1b[01;33mwarning: \x1b[0m未知的奖项名称：\x1b[0;32m\'{level}\'\x1b[0m', file=stderr)
            return level
    ...
    def to_compress_format(self, reference_em):
        ...
        s = '{}:{}:{}:{}:{}:{}'.format(
            self.contest.id, self.school.id, Record.__score_format__(
                self.score),
            self.rank, Record.__province_format__(
                self.province), Record.__award_level_format__(self.level)
        )
    ...
    def to_highly_compressed_format(self, reference_em):
        '转化成压缩格式字符串。'

        s = '{}_{}'.format(
            self.contest.id, Record.__award_level_format__(self.level)
        )
        return s
    ...
    def check_stay_down(A, B):
        ...
        locations = set(record.school.location() for record in chain(A, B))
        provinces = set(record.province for record in chain(A, B))
        penalty = __school_penalty__.get(
            len(schools), 600) + 80 * (len(locations) + len(provinces) - 3)
        if penalty >= 100:
            return 0
